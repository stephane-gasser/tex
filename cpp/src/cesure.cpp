...
static smallnumber reconstitute(smallnumber j, smallnumber n, halfword bchar, halfword hchar, const std::basic_string<halfword> &word)
{
	...
	if (j == 0)
	{
		...
		for (auto p = initlist; p; next(p))
			appendAtEnd(t, new CharNode(hf, p->character)); // Head -> t [curl] -> 0
	}
	else 
		if (curl < non_char)
			appendAtEnd(t, new CharNode(hf, curl)); // Head -> t [curl] -> 0
	ligstack = nullptr;
...
void hyphenate(LinkedNode *curp, std::basic_string<halfword> word)
{
	...
	while (j <= word.size()-1)
	{
		...
		if (hyphenpassed == 0)
		{
			followUntilEnd(&Head, s); // Head -> ... -> s -> null
			if (hyf[j-1]%2)
			{
				l = j;
				hyphenpassed = j-1;
			...
			do
			{
				...
				while (l <= i)
				{
					...
					if (Head.link)
					{
						if (minortail == nullptr)
							dynamic_cast<DiscNode*>(r)->pre_break = Head.link;
						else
							minortail->link = Head.link;
						followUntilEnd(&Head, minortail);
					}
				}
				if (hyfnode)
				{
				...
				while (l < j)
				{
					...
					while (l > j)
					{
						...
						for (majortail = &Head; majortail->link; next(majortail)) //advance_major_tail
							rcount++;
					}
				}
...
void newpatterns(char status, Token t)
{
	...
	if (curlang <= 0 || curlang > 255)
			curlang = 0;
	scanner.leftBrace(status); //a left brace must follow \\patterns
	hyf[0] = 0;
...
void newhyphexceptions(char status)
{
	...
	for (auto t = scanner.getX(status); true; t = scanner.getX(status))
		switch (t.cmd)
		{
			case char_num:
