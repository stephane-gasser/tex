...
var
  ...
  {:30}{39:}strpool: packed array[poolpointer] of packedASCIIcode;
...
procedure closefilesandterminate; forward;
...
begin
  ...
  while p <> 0 do
  begin
    if (p > memend) or (p < himemmin) then
      clobbered := true
    else
    if Free[p] then
      clobbered := true;
  ...
  clobbered := false;
  repeat
    if (p >= lomemmax) or (p < memmin) then
      clobbered := true
    else if (mem[p + 1].hh.rh >= lomemmax) or (mem[p + 1].hh.rh < memmin) then
      clobbered := true
    else if not ((mem[p].hh.rh = 65535)) or (mem[p].hh.lh < 2) or (p + mem[p].hh.lh > lomemmax) or (mem[mem[p + 1].hh.rh + 1].hh.lh <> p) then
      clobbered := true;
    if clobbered then
    begin
      printnl(302);
  ...
  begin
    printnl(305);
    for p := memmin to lomemmax do
      if not Free[p] and ((p > waslomax) or wasfree[p]) then
      begin
        printchar(32);
        printint(p);
      end;
    for p := himemmin to memend do
      if not Free[p] and ((p < washimin) or (p > wasmemend) or wasfree[p]) then
      begin
        printchar(32);
        printint(p);
      end;
...
begin
  ...
  {285:}if saveptr > 0 then
    for q := 0 to saveptr - 1 do
      if savestack[q].hh.rh = p then
      begin
        printnl(546);
        printint(q);
        printchar(41);
      end{:285};
  {933:}for q := 0 to 307 do
...
begin
  ...
  begin
    ...
    {229:}if n < 2900 then
    begin
      printskipparam(n - 2882);
      printchar(61);
      if n < 2897 then
        printspec(eqtb[n].hh.rh, 397)
      else
        printspec(eqtb[n].hh.rh,
          337);
    end
    else if n < 3156 then
    begin
    ...
    begin
      ...
      if eqtb[3412].hh.rh = 0 then
        ...
        printint(mem[eqtb[3412].hh.rh].hh.lh);
    end
    else if n < 3422 then
    begin
      printcmdchr(72, n);
      printchar(61);
      if eqtb[n].hh.rh <> 0 then
        showtokenlist(mem[eqtb[n].hh.rh].hh.rh, 0, 32);
    end
    else if n < 3678 then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if eqtb[5300].int > 0 then
          restoretrace(p, 544){:283};
      end;
    end;
    30:
    ...
    25:
      ...
      21:
        ...
        {344:}case curinput.statefield + curcmd of
          {345:}10, 26, 42, 27, 43{:345}: goto 25;
          1, 17, 33:{354:}
          begin
            if curinput.locfield > curinput.limitfield then
              curcs := 513
...
begin
  if fontbc[f] <= c then
    if fontec[f] >= c then
      if (fontinfo[charbase[f] + c].qqqq.b0 > 0) then
      begin
        p := getavail;
        mem[p].hh.b0 := f;
        mem[p].hh.b1 := c;
...
begin
  ...
  hd := q.b1;
  b := newnullbox;
  mem[b + 1].int := fontinfo[widthbase[f] + q.b0].int + fontinfo[italicbase[f] + (q.b2) div 4].int;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                print(927);
                if mem[passive + 1].hh.lh = 0 then
                  printchar(48)
                else
                  printint(mem[mem[passive + 1].hh.lh].hh.lh);
              end{:846};
            end{:845};
    ...
    begin
      ...
      begin
        ...
        if curp = 0 then
          ...
          if mem[curp].hh.b0 = 12 then
            printesc(531)
          else if mem[curp].hh.b0 = 7 then
            printesc(349)
          else if mem[curp].hh.b0 = 11 then
            printesc(340)
          else
            printesc(
              343);
        print(928);
        if mem[r + 1].hh.rh = 0 then
          printchar(48)
        else
          printint(mem[mem[r + 1].hh.rh].hh.lh);
        print(929);
        if b > 10000 then
          printchar(42)
        else
...
{:829}{877:}
procedure postlinebreak(finalwidowpenalty: integer; nonprunablep: halfword);
label
  30, 31;
var
  q, r, s: halfword;
...
var
  ...
  45:
    hn := hn - 1{:930};
  if trie[curlang + 1].b1 <> curlang then
    goto 10;
  hc[0] := 0;
  hc[hn + 1] := 0;
  hc[hn + 2] := 256;
...
var
  ...
  {:863};
  {876:}postlinebreak(finalwidowpenalty, nonprunablep){:876};
  {865:}q := mem[29993].hh.rh;
  while q <> 29993 do
  begin
    curp := mem[q].hh.rh;
...
begin
  ...
  begin
    ...
    f := mem[p].hh.b0;
    begin
      mem[curlist.tailfield].hh.rh := newkern(fontinfo[italicbase[f] + (fontinfo[charbase[f] + mem[p].hh.b1].qqqq.b2) div 4].int);
      curlist.tailfield := mem[curlist.tailfield].hh.rh;
    end;
  ...
  80:
    ...
    begin
      if mem[curq].hh.rh > 0 then
        if mem[curlist.tailfield].hh.b1 = hyphenchar[mainf] then
          insdisc := true;
      if ligaturepresent then
      begin
  ...
  if eqtb[5294].int > 0 then
    ...
    begin
      ...
      writeln(logfile, ' ', maxinstack: 1, 'i,', maxneststack: 1, 'n,', maxparamstack:
        1, 'p,', maxbufstack + 1: 1, 'b,', maxsavestack + 6: 1, 's stack positions out of ', stacksize: 1, 'i,', nestsize: 1, 'n,', paramsize: 1, 'p,', bufsize: 1, 'b,',
        savesize: 1, 's');
    end{:1334};
  ;
  {642:}while curs > -1 do
  ...
  begin
    ...
    begin
      ...
      case m of
        ...
        16: panicking := not panicking;{:1339}
        else print(63)
      end;
    end;
  end;
...
{:1338}{:1330}{1332:}begin
  ...
  readyalready := 314159;
  1:
    {1379:}if formatident = 0 then
    begin
      packbufferedname(16, 1, 0);
      if not wopenin(fmtfile) then
      begin
        writeln(output,
        'I can''t find the format file ', nameoffile);
        goto 9999;
      end;
  ...
  termoffset := 0;
  fileoffset := 0;
  {:55}{61:}write(output, 'This is TeX-FPC');
